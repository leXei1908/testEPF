...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ОбъектыНазначения) Экспорт
	Если ИдентификаторКоманды = "ЗаполнитьЧерепицу" Тогда
		ОповещениеПослеЗакрытия = Новый ОписаниеОповещения("ДобавитьРазмерыВДокумент", ЭтотОбъект);
...
Процедура ДобавитьРазмерыВДокумент(РезультатЗакрытия, Параметры) Экспорт
	Если ТипЗнч(РезультатЗакрытия) = Тип("ДанныеФормыКоллекция") Тогда
		...
		Для Каждого СтрокаДанных Из РезультатЗакрытия Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтруктураНДС);
		КонецЦикла;
	КонецЕсли;
...
//		ВладелецФормы.ИмяФормы, "Товары"));
	...
	СтруктураСтроки.Вставить("СуммаБезВозвратнойТары", ТекущаяСтрока.Сумма);
	СтруктураСтроки.Вставить("СуммаНДС", ТекущаяСтрока.СуммаНДС);
	СтруктураСтроки.Вставить("СуммаСНДС", ТекущаяСтрока.СуммаСНДС);
	СтруктураСтроки.Вставить("СуммаСНДСБезВозвратнойТары", ТекущаяСтрока.СуммаСНДС);
	СтруктураСтроки.Вставить("Цена", ТекущаяСтрока.Цена);
	СтруктураСтроки.Вставить("КоличествоУпаковок", ТекущаяСтрока.КоличествоУпаковок);
	СтруктураСтроки.Вставить("Количество", ТекущаяСтрока.Количество);
	СтруктураСтроки.Вставить("БезВозвратнойТары", ТекущаяСтрока.БезВозвратнойТары);
	СтруктураСтроки.Вставить("ОтмененоБезВозвратнойТары", ТекущаяСтрока.ОтмененоБезВозвратнойТары);
	СтруктураСтроки.Вставить("Отменено", ТекущаяСтрока.Отменено);
